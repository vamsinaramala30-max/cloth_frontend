...
export default function ProductFormClient({
  ...
  const setField = <K extends keyof AdminProductInput>(key: K, val: AdminProductInput[K]) => {
    setValues((v) => ({ ...v, [key]: val }));
  };

  const setVariant = (idx: number, patch: Partial<AdminProductVariantInput>) => {
    setValues((v) => {
  ...
  const addVariant = () => setValues((v) => ({ ...v, variants: [...v.variants, { ...defaultVariant }] }));

  const removeVariant = (idx: number) =>
    setValues((v) => ({
      ...v,
      variants: v.variants.length <= 1 ? v.variants : v.variants.filter((_, i) => i !== idx),
    }));
  ...
  const validate = (): string | null => {
    ...
    if (!Array.isArray(values.variants) || values.variants.length === 0) return 'At least one variant is required.';
    ...
    for (let i = 0; i < values.variants.length; i++) {
      ...
      if (!v.colorName.trim()) return `Variant #${i + 1}: Color name is required.`;
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="md:col-span-2">
          ...
          <p className="text-xs text-zinc-400 mt-1">Create a new product document (Mongoose Product schema).</p>
        </div>

        <div className="md:col-span-1">
          <label className="block text-xs font-bold tracking-[0.2em] text-white uppercase mb-2">Name</label>
          <input
            value={values.name}
        ...
        <div className="md:col-span-1">
          <label className="block text-xs font-bold tracking-[0.2em] text-white uppercase mb-2">Slug</label>
          <input
            value={values.slug}
            ...
            placeholder="e.g. quantum-silk-jacket"
          />
          <p className="text-[11px] text-zinc-500 mt-1">Auto-generated from name if empty.</p>
        </div>

        <div className="md:col-span-2">
          <label className="block text-xs font-bold tracking-[0.2em] text-white uppercase mb-2">Description</label>
          <textarea
        ...
        <div>
          <label className="block text-xs font-bold tracking-[0.2em] text-white uppercase mb-2">Base Price</label>
          <input
            type="number"
            value={values.basePrice}
            onChange={(e) => setField('basePrice', e.target.value === '' ? '' : Number(e.target.value))}
            className="w-full bg-white/5 border border-white/15 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-cyan-400"
          />
        </div>

        <div>
          <label className="block text-xs font-bold tracking-[0.2em] text-white uppercase mb-2">Sale Price (optional)</label>
          <input
            type="number"
            value={values.salePrice}
            onChange={(e) => setField('salePrice', e.target.value === '' ? '' : Number(e.target.value))}
            className="w-full bg-white/5 border border-white/15 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-cyan-400"
          />
        </div>
          ...
          {values.variants.map((v, idx) => (
            <div key={idx} className="p-5 rounded-xl bg-white/5 border border-white/10">
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold tracking-[0.2em] text-white uppercase mb-2">SKU</label>
                  <input
                    value={v.sku}
                ...
                <div>
                  <label className="block text-xs font-bold tracking-[0.2em] text-white uppercase mb-2">Size</label>
                  <select
                    value={v.size}
                ...
                <div>
                  <label className="block text-xs font-bold tracking-[0.2em] text-white uppercase mb-2">Color</label>
                  <input
                    value={v.color}
                ...
                <div>
                  <label className="block text-xs font-bold tracking-[0.2em] text-white uppercase mb-2">Color Name</label>
                  <input
                    value={v.colorName}
                ...
                <div>
                  <label className="block text-xs font-bold tracking-[0.2em] text-white uppercase mb-2">Stock</label>
                  <input
                    type="number"
                    min={0}
                    value={v.stock}
                ...
                <div className="md:col-span-1 md:col-start-2">
                  <label className="block text-xs font-bold tracking-[0.2em] text-white uppercase mb-2">Images (comma separated URLs)</label>
                  <input
                    value={v.images.join(', ')}
                    onChange={(e) =>
                      setVariant(idx, {
      ...
      <div className="mt-10 flex items-center justify-end gap-3">
        <button
          type="submit"
          disabled={loading}
          className="px-6 py-3 rounded-lg bg-cyan-400/15 border border-cyan-400/40 text-cyan-200 hover:bg-cyan-400/20 transition disabled:opacity-50"
        >
